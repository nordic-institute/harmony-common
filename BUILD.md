...
| `harmony.deb.builder.pullPolicy` | When to pull the builder image: `always`, `if-not-present`, `never` | `if-not-present`                                                       |
...
| `harmony.docker.pullPolicy`        | When to pull base images in Dockerfile: `always`, `if-not-present`, `never` | `if-not-present`            |
...
| `if-not-present` | Pull only if image doesn't exist locally. Recommended for local development. |
